 ...
 *
 */
if ( ! class_exists( 'CHIPFLUENT_Shortcoder' ) ) {
  class CHIPFLUENT_Shortcoder extends CHIPFLUENT_Abstract{
    ...
    public $args         = array(
      ...
      'class'            => '',
      'gutenberg'        => array(
        'title'          => 'CHIPFLUENT Shortcodes',
        'description'    => 'CHIPFLUENT Shortcode Block',
        'icon'           => 'screenoptions',
        'category'       => 'widgets',
        'keywords'       => array( 'shortcode', 'chipfluent', 'insert' ),
    ...
    public function __construct( $key, $params = array() ) {
      ...
      add_action( 'wp_ajax_chipfluent-get-shortcode-'. $this->unique, array( $this, 'get_shortcode' ) );

      if ( ! empty( $this->args['show_in_editor'] ) ) {

        $name = str_replace( '_', '-', sanitize_title( $this->unique ) );

        CHIPFLUENT::$shortcode_instances[] = wp_parse_args( array( 'name' => 'chipfluent/'. $name, 'modal_id' => $this->unique ), $this->args );

        // elementor editor support
        if ( CHIPFLUENT::is_active_plugin( 'elementor/elementor.php' ) ) {
          add_action( 'elementor/editor/before_enqueue_scripts', array( 'CHIPFLUENT', 'add_admin_enqueue_scripts' ) );
          add_action( 'elementor/editor/footer', array( 'CHIPFLUENT_Field_icon', 'add_footer_modal_icon' ) );
    ...
    public function add_footer_modal_shortcode() {
      ...
      if( ! wp_script_is( 'chipfluent' ) ) {
        return;
      }

      $class        = ( $this->args['class'] ) ? ' '. esc_attr( $this->args['class'] ) : '';
      $has_select   = ( count( $this->pre_tabs ) > 1 ) ? true : false;
      $single_usage = ( ! $has_select ) ? ' chipfluent-shortcode-single' : '';
      $hide_header  = ( ! $has_select ) ? ' hidden' : '';

    ?>
      <div id="chipfluent-modal-<?php echo esc_attr( $this->unique ); ?>" class="wp-core-ui chipfluent-modal chipfluent-shortcode hidden<?php echo esc_attr( $single_usage . $class ); ?>" data-modal-id="<?php echo esc_attr( $this->unique ); ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( 'chipfluent_shortcode_nonce' ) ); ?>">
          ...
          <div class="chipfluent-modal-table-cell">
            ...
            <div class="chipfluent-modal-inner">
              <div class="chipfluent-modal-title">
                ...
                <div class="chipfluent-modal-close"></div>
              </div>
              <?php

                echo '<div class="chipfluent-modal-header'. esc_attr( $hide_header ) .'">';
                echo '<select>';
              ...
              <div class="chipfluent-modal-insert-wrapper hidden"><a href="#" class="button button-primary chipfluent-modal-insert"><?php echo $this->args['insert_title']; ?></a></div>
            </div>
          </div>
        </div>
      </div>
    ...
    public function get_shortcode() {
      ...
      if ( ! empty( $shortcode_key ) && wp_verify_nonce( $nonce, 'chipfluent_shortcode_nonce' ) ) {
        ...
        if ( ! empty( $section ) ) {
          ...
          if ( ! empty( $section['fields'] ) && $view !== 'repeater' ) {
            ...
            echo '<div class="chipfluent-fields">';
            ...
            foreach ( $section['fields'] as $field ) {
              ...
              CHIPFLUENT::field( $field, $field_default, $shortcode, 'shortcode' );
          ...
          if ( ! empty( $repeatable_fields ) ) {
            ...
            $button_title    = ( ! empty( $section['button_title'] ) ) ? ' '. $section['button_title'] : esc_html__( 'Add New', 'chipfluent' );
            $inner_shortcode = ( ! empty( $section['group_shortcode'] ) ) ? $section['group_shortcode'] : $shortcode;

            echo '<div class="chipfluent--repeatable">';

              echo '<div class="chipfluent--repeat-shortcode">';

                echo '<div class="chipfluent-repeat-remove fas fa-times"></div>';
                ...
                foreach ( $repeatable_fields as $field ) {
                  ...
                  CHIPFLUENT::field( $field, $field_default, $inner_shortcode.'[0]', 'shortcode' );
            ...
            echo '<div class="chipfluent--repeat-button-block"><a class="button chipfluent--repeat-button" href="#"><i class="fas fa-plus-circle"></i> '. $button_title .'</a></div>';
        ...
        echo '<div class="chipfluent-field chipfluent-error-text">'. esc_html__( 'Error: Invalid nonce verification.', 'chipfluent' ) .'</div>';
      }
    ...
    public static function once_editor_setup() {
      ...
      if ( function_exists( 'register_block_type' ) ) {
        add_action( 'enqueue_block_editor_assets', array( 'CHIPFLUENT_Shortcoder', 'add_guteberg_blocks' ) );
      }

      if ( chipfluent_wp_editor_api() ) {
        add_action( 'media_buttons', array( 'CHIPFLUENT_Shortcoder', 'add_media_buttons' ) );
      }
    ...
    public static function add_guteberg_blocks() {
      ...
      wp_enqueue_script( 'chipfluent-gutenberg-block', CHIPFLUENT::include_plugin_url( 'assets/js/gutenberg.js' ), $depends );

      wp_localize_script( 'chipfluent-gutenberg-block', 'chipfluent_gutenberg_blocks', CHIPFLUENT::$shortcode_instances );

      foreach ( CHIPFLUENT::$shortcode_instances as $block ) {

        register_block_type( $block['name'], array(
          'editor_script' => 'chipfluent-gutenberg-block',
        ) );
    ...
    public static function add_media_buttons( $editor_id ) {
      ...
      foreach ( CHIPFLUENT::$shortcode_instances as $value ) {
        echo '<a href="#" class="button button-primary chipfluent-shortcode-button" data-editor-id="'. esc_attr( $editor_id ) .'" data-modal-id="'. esc_attr( $value['modal_id'] ) .'">'. esc_html( $value['button_title'] ) .'</a>';
      }
