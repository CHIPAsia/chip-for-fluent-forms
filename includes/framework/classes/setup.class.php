...
if (!class_exists('CHIPFLUENT_Setup')) {
  class CHIPFLUENT_Setup
  {
    ...
    public function __construct()
    {
      ...
      // Init action
      do_action('chipfluent_init');

      // Setup textdomain
      self::textdomain();

      add_action('after_setup_theme', array('CHIPFLUENT', 'setup'));
      add_action('init', array('CHIPFLUENT', 'setup'));
      add_action('switch_theme', array('CHIPFLUENT', 'setup'));
      add_action('admin_enqueue_scripts', array('CHIPFLUENT', 'add_admin_enqueue_scripts'));
      add_action('wp_enqueue_scripts', array('CHIPFLUENT', 'add_typography_enqueue_styles'), 80);
      add_action('wp_head', array('CHIPFLUENT', 'add_custom_css'), 80);
      add_filter('admin_body_class', array('CHIPFLUENT', 'add_admin_body_class'));
    ...
    public static function setup()
    {
      ...
      $params = array();
      if (class_exists('CHIPFLUENT_Options') && !empty(self::$args['admin_options'])) {
        foreach (self::$args['admin_options'] as $key => $value) {
          if (!empty(self::$args['sections'][$key]) && !isset(self::$inited[$key])) {
            ...
            CHIPFLUENT_Options::instance($key, $params);
    ...
    public static function include_plugin_file($file, $load = true)
    {
      ...
      $path = '';
      $file = ltrim($file, '/');
      $override = apply_filters('chipfluent_override', 'chipfluent-override');
    ...
    public static function includes()
    {
      ...
      $fields = apply_filters('chipfluent_fields', array(
        'accordion',
        'background',
        'backup',
        'border',
      ...
      if (!empty($fields)) {
        foreach ($fields as $field) {
          if (!class_exists('CHIPFLUENT_Field_' . $field) && class_exists('CHIPFLUENT_Fields')) {
    ...
    {
      load_textdomain('chipfluent', self::$dir . '/languages/' . get_locale() . '.mo');
    }
    ...
    public static function add_admin_enqueue_scripts()
    {
      ...
      if (!self::$enqueue) {
        ...
        if ($wpscreen->id === 'tools_page_chipfluent-welcome') {
          self::$enqueue = true;
        }

      }

      if (!apply_filters('chipfluent_enqueue_assets', self::$enqueue)) {
        return;
      }
      ...
      if (apply_filters('chipfluent_fa4', false)) {
        wp_enqueue_style('chipfluent-fa', 'https://cdn.jsdelivr.net/npm/font-awesome@4.7.0/css/font-awesome.min.css', array(), '4.7.0', 'all');
      } else {
        wp_enqueue_style('chipfluent-fa5', 'https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@5.15.4/css/all.min.css', array(), '5.15.5', 'all');
        wp_enqueue_style('chipfluent-fa5-v4-shims', 'https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@5.15.4/css/v4-shims.min.css', array(), '5.15.5', 'all');
      }
      ...
      // Main style
      wp_enqueue_style('chipfluent', self::include_plugin_url('assets/css/style' . $min . '.css'), array(), self::$version, 'all');

      // Main RTL styles
      if (is_rtl()) {
        wp_enqueue_style('chipfluent-rtl', self::include_plugin_url('assets/css/style-rtl' . $min . '.css'), array(), self::$version, 'all');
      }

      // Main scripts
      wp_enqueue_script('chipfluent-plugins', self::include_plugin_url('assets/js/plugins' . $min . '.js'), array(), self::$version, true);
      wp_enqueue_script('chipfluent', self::include_plugin_url('assets/js/main' . $min . '.js'), array('chipfluent-plugins'), self::$version, true);

      // Main variables
      wp_localize_script('chipfluent', 'chipfluent_vars', array(
        'color_palette' => apply_filters('chipfluent_color_palette', array()),
        'i18n' => array(
          'confirm' => esc_html__('Are you sure?', 'chipfluent'),
          'typing_text' => esc_html__('Please enter %s or more characters', 'chipfluent'),
          'searching_text' => esc_html__('Searching...', 'chipfluent'),
          'no_results_text' => esc_html__('No results found.', 'chipfluent'),
        ),
      ));
      ...
      if (!empty(self::$fields)) {
        foreach (self::$fields as $field) {
          if (!empty($field['type'])) {
            $classname = 'CHIPFLUENT_Field_' . $field['type'];
    ...
    public static function add_typography_enqueue_styles()
    {
      ...
      if (!empty(self::$webfonts)) {
        ...
        if (!empty(self::$webfonts['enqueue'])) {
          ...
          wp_enqueue_style('chipfluent-google-web-fonts', esc_url(add_query_arg($query, '//fonts.googleapis.com/css')), array(), null);
        ...
        if (!empty(self::$webfonts['async'])) {
          ...
          wp_enqueue_script('chipfluent-google-web-fonts', esc_url('//ajax.googleapis.com/ajax/libs/webfont/1.6.26/webfont.js'), array(), null);

          wp_localize_script('chipfluent-google-web-fonts', 'WebFontConfig', array('google' => array('families' => $fonts)));
    ...
    public static function field($field = array(), $value = '', $unique = '', $where = '', $parent = '')
    {
      ...
      if (!empty($field['_notice'])) {
        ...
        $field = array();
        $field['content'] = esc_html__('Oops! Not allowed.', 'chipfluent') . ' <strong>(' . $field_type . ')</strong>';
        $field['type'] = 'notice';
        $field['style'] = 'danger';
      ...
      $is_pseudo = (!empty($field['pseudo'])) ? ' chipfluent-pseudo-field' : '';
      ...
      if (!empty($field['dependency'])) {
        ...
        $visible = (!empty($depend_visible)) ? ' chipfluent-depend-visible' : ' chipfluent-depend-hidden';

      }

      // These attributes has been sanitized above.
      echo '<div class="chipfluent-field chipfluent-field-' . $field_type . $is_pseudo . $class . $visible . '"' . $depend . '>';

      if (!empty($field_type)) {

        if (!empty($field['title'])) {
          echo '<div class="chipfluent-title">';
          echo '<h4>' . $field['title'] . '</h4>';
          echo (!empty($field['subtitle'])) ? '<div class="chipfluent-subtitle-text">' . $field['subtitle'] . '</div>' : '';
          echo '</div>';
        }

        echo (!empty($field['title'])) ? '<div class="chipfluent-fieldset">' : '';

        $value = (!isset($value) && isset($field['default'])) ? $field['default'] : $value;
        $value = (isset($field['value'])) ? $field['value'] : $value;

        $classname = 'CHIPFLUENT_Field_' . $field_type;

        if (class_exists($classname)) {
          $instance = new $classname($field, $value, $unique, $where, $parent);
          $instance->render();
        } else {
          echo '<p>' . esc_html__('Field not found!', 'chipfluent') . '</p>';
        }
