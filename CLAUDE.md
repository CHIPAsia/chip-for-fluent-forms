...
- **Option keys**: `fluent_form_chip_settings` (global, the canonical store; mirrored to FF Pro's `fluentform_payment_settings_chip` on save), per-form `fluentform_form_meta` rows with `meta_key = '_chip_payment_settings'`. The form's `payment_method` field's `settings.payment_methods[chip]` sub-array (lives in `fluentform_forms.form_fields` JSON) is what the form editor / render consults.
...
- **`class-chip-fluent-forms-form-settings.php`** — Per-form "Customize" panel. Hooked on `fluentform/form_payment_settings` to add a per-form payment-mode override and a per-form payment-method whitelist. **Note**: FF Pro's React form-settings UI does not currently render the per-method subkeys added by this filter, so the panel is invisible in the form editor. The render filter is kept as a future extension point and for downstream integrations.
...
At purchase time the runtime calls `Chip_Fluent_Forms_Purchase::resolve_effective_config($form_id, $methodSettings)` which merges three layers in priority order:

1. **Per-form `payment_method` field settings** (highest priority) — read from `$methodSettings['settings']`, the per-method field settings that FF Pro persists into the form's `form_fields` JSON. The merchant sets these in the form editor by clicking the `payment_method` field and editing the `chip` sub-panel alongside Method Label and Notes. The customizer toggle (`is_active='yes'`) is what gates the override.
2. **`_chip_payment_settings` row** (fallback) — read via `Chip_Fluent_Forms_Settings::for_form($form_id)`. The migration creates these for 1.x → 2.0 upgraded users. If a form's per-method field has no override (e.g. it was created in 1.x and never re-saved in 2.0), the runtime uses these rows.
3. **Global `fluent_form_chip_settings`** (lowest priority) — the values from the FF Pro Payment Methods tab (`brand_id`, `secret_key`, `payment_mode`, `due_strict`, `due_strict_timing`, `payment_method_whitelist`).

If the per-form method field's `is_active` toggle is off (or missing), the credential keys fall back to the lower layers. `option_label` and `notes` always use the per-form method field's value (they have no global equivalent).
