...
- **`class-chip-fluent-forms-per-form-page.php`** — Dedicated per-form admin page (`wp-admin/admin.php?page=chip-form-settings`, registered as a top-level menu, cap `manage_options`). The list view shows every FF form with a Customized Yes/No column. The per-form edit view (`&form_id=N`) renders a WP Settings API form with seven fields (Customize toggle, Payment Mode, Brand ID, Secret Key, Due Strict, Due Strict Timing, Payment Method Whitelist). Save POSTs to `wp-admin/admin-post.php?action=chip_save_per_form_settings` → `Chip_Fluent_Forms_Settings::save_form()` → `_chip_payment_settings` row in `fluentform_form_meta`. The top-level menu (not a submenu of `fluent_forms`) is required because FF Pro's parent menu gates on `fluentform_dashboard_access`, which stock admins don't have.
